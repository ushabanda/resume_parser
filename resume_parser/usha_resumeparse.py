# %%writefile /content/resume_parser/resume_parser/true_talent_resumeparse.py
...
class true_talent_resumeparse(object):
    ...
    def find_segment_indices(string_to_search, resume_segments, resume_indices):
        for i, line in enumerate(string_to_search):
            ...
            if [o for o in true_talent_resumeparse.objective if header.startswith(o)]:
                try:
                    resume_segments['objective'][header]
                except:
                    resume_indices.append(i)
                    header = [o for o in true_talent_resumeparse.objective if header.startswith(o)][0]
                    resume_segments['objective'][header] = i
            elif [w for w in true_talent_resumeparse.work_and_employment if header.startswith(w)]:
                try:
                    resume_segments['work_and_employment'][header]
                except:
                    resume_indices.append(i)
                    header = [w for w in true_talent_resumeparse.work_and_employment if header.startswith(w)][0]
                    resume_segments['work_and_employment'][header] = i
            elif [e for e in true_talent_resumeparse.education_and_training if header.startswith(e)]:
                try:
                    resume_segments['education_and_training'][header]
                except:
                    resume_indices.append(i)
                    header = [e for e in true_talent_resumeparse.education_and_training if header.startswith(e)][0]
                    resume_segments['education_and_training'][header] = i
            elif [s for s in true_talent_resumeparse.skills_header if header.startswith(s)]:
                try:
                    resume_segments['skills'][header]
                except:
                    resume_indices.append(i)
                    header = [s for s in true_talent_resumeparse.skills_header if header.startswith(s)][0]
                    resume_segments['skills'][header] = i
            elif [m for m in true_talent_resumeparse.misc if header.startswith(m)]:
                try:
                    resume_segments['misc'][header]
                except:
                    resume_indices.append(i)
                    header = [m for m in true_talent_resumeparse.misc if header.startswith(m)][0]
                    resume_segments['misc'][header] = i
            elif [a for a in true_talent_resumeparse.accomplishments if header.startswith(a)]:
                try:
                    resume_segments['accomplishments'][header]
                except:
                    resume_indices.append(i)
                    header = [a for a in true_talent_resumeparse.accomplishments if header.startswith(a)][0]
    ...
    def segment(string_to_search):
        ...
        true_talent_resumeparse.find_segment_indices(string_to_search, resume_segments, resume_indices)
        if len(resume_indices) != 0:
            true_talent_resumeparse.slice_segments(string_to_search, resume_segments, resume_indices)
        else:
    ...
    def get_experience(resume_segments):
        ...
        if len(resume_segments['work_and_employment'].keys()):
            ...
            for key, values in resume_segments['work_and_employment'].items():
                text += " ".join(values) + " "
            total_exp = true_talent_resumeparse.calculate_experience(text)
    ...
    def read_file(self, file,docx_parser = "tika"):
        ...
        if file.endswith('docx') or file.endswith('doc'):
            ...
            resume_lines, raw_text = true_talent_resumeparse.convert_docx_to_txt(file,docx_parser)
        elif file.endswith('pdf'):
            resume_lines, raw_text = true_talent_resumeparse.convert_pdf_to_txt(file)
            ...
            resume_lines = None
        resume_segments = true_talent_resumeparse.segment(resume_lines)
        
        
        full_text = " ".join(resume_lines)

        email = true_talent_resumeparse.extract_email(full_text)
        phone = true_talent_resumeparse.find_phone(full_text)
        name = true_talent_resumeparse.extract_name(" ".join(resume_segments['contact_info']))
        total_exp, text = true_talent_resumeparse.get_experience(resume_segments)
        university = true_talent_resumeparse.extract_university(full_text, os.path.join(base_path,'world_universities.csv'))

        designition = true_talent_resumeparse.job_designition(full_text)
        designition = list(dict.fromkeys(designition).keys())

        degree = true_talent_resumeparse.get_degree(full_text)
        company_working = true_talent_resumeparse.get_company_working(full_text)
        # experience_dict=true_talent_resumeparse.get_work_and_employment(full_text)
        skills = ""
        ...
        if len(skills) == 0:
            skills = true_talent_resumeparse.extract_skills(full_text)
...
# resume_text = """
#     Name - Test user. phone is 8912342345. email is aadgsd@gmail.com. 
#     Total 9+ Years of experience as a Stress Analyst (8 + Years) and Design Engineer (1+ Year) of experience as a Design Engineer.

# Education is  M.Tech (Master’s in Technology) in Mechanical Engineering at Dayananda Sagar College of engineering, Bangalore, India.

# Skill are Modelling and computational software like ANSYS Workbench, ANSYS Classic, SIESTA, Abaqus, NX, Hypermesh, Hyper view, Catia, Solid Works and Pro-e.

# worked at 
#      1. Belcan India Pvt Limited, Hyderabad from December 2021 to August 2022 as Lead Engineer.
#      2. Quest Global Engineering Services Pvt Limited Bangalore from April 2017 to May 2020 as Senior Engineer.
#      3. HCL Technologies Limited, Pune from June2014 to April 2017 as Senior Engineer.
#      4. ASM Technologies, Bangalore from October 2012 to February 2014 as Mechanical Design Engineer.
#      5. Glaxion Technologies, Bangalore from July 2006 to September 2007 as Design Engineer.
              
# """

# parser_obj = true_talent_resumeparse()
# # parsed_resume_data = parser_obj.read_file('C:\\Users\\Administrator\\Downloads\\rajesh_cv.pdf')
# parsed_resume_data = parser_obj.read_file('C:\\Users\\Administrator\\Desktop\\Usha_resume_parser\\resume.txt')
